...
export const DebtManagement: React.FC = () => {
  ...
  const statusOptions = [
    ...
    {value: "open", label: "Đang mở", color: "default"},
    ...
    {value: "settled", label: "Đã tất toán hết", color: "success"},
  ];
  ...
  const loadDebts = useCallback(
    async (searchParams?: DebtSearchParams) => {
      try {
        setLoading(true);
        setError(null);
          ...
          if (activeTab === "customers" && newDebts.length > 0) {
            const ids = Array.from(
              new Set(
                newDebts
                  .map((d: any) =>
                    d?.customer_id && typeof d.customer_id !== "object"
            ...
            if (ids.length > 0) {
              try {
                const results = await Promise.allSettled(
                  ids.map((id) => customerService.getCustomerById(id))
                );
                const map: Record<
                  string,
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const totalDebt = debts.reduce(
        (sum, debt: any) => sum + toNumber(debt.total_amount, 0),
        0
      );
      const totalPaid = debts.reduce(
        (sum, debt) => sum + getPaidAmount(debt),
        0
      );
      const totalRemaining = debts.reduce(
        (sum, debt: any) => sum + toNumber(debt.remaining_amount, 0),
        0
      );
  ...
  const handleViewDebt = async (debt: Debt) => {
    ...
    try {
      if (activeTab === "customers") {
        ...
        const id =
          (cid && typeof cid === "object" ? cid._id : cid) ||
          (debt as any).debtor_id;
        if (id) {
