...
import { Bike, Battery, Zap, Clock, Eye, ShoppingCart, X, Search, SlidersHorizontal } from 'lucide-react';
...
export const Motorbike: React.FC = () => {
  ...
  const loadVehicles = async () => {
    try {
      ...
      console.log('🚀 Loading motorbikes from API...');
      ...
      if (response.success && (response as Record<string, unknown>).data) {
        ...
        console.log('✅ Motorbikes loaded successfully:', responseData.data);
        const motorbikes = responseData.data as unknown[];
        console.log('📊 Total motorbikes from API:', motorbikes.length);
        console.log('📋 Motorbikes data:', motorbikes);
        setAllVehicles(motorbikes);
        setFilteredVehicles(motorbikes);
      } else {
        console.error('❌ Failed to load motorbikes:', response.message);
        setError(response.message || 'Không thể tải danh sách xe máy');
        // Fallback to mock data
        setAllVehicles(mockMotorbikes);
  ...
  const applyFilters = useCallback(() => {
    ...
    console.log('🔍 Starting filter with', allVehicles.length, 'motorbikes');
    ...
    if (filters.search) {
      ...
      console.log('🔍 After search filter:', filtered.length, 'motorbikes');
    }
    ...
    filtered = filtered.filter((vehicle: unknown) => {
      ...
      if (!inRange) {
        console.log('🚫 Motorbike filtered out by price:', v.name, 'price:', price, 'range:', filters.priceRange);
      }
      return inRange;
    });
    console.log('🔍 After price filter:', filtered.length, 'motorbikes');
    ...
    filtered = filtered.filter((vehicle: unknown) => {
      ...
      if (!inRange) {
        console.log('🚫 Motorbike filtered out by range:', v.name, 'range:', range, 'range filter:', filters.rangeKm);
      }
      return inRange;
    });
    console.log('🔍 After range filter:', filtered.length, 'motorbikes');
    ...
    filtered = filtered.filter((vehicle: unknown) => {
      ...
      if (!inRange) {
        console.log('🚫 Motorbike filtered out by motor power:', v.name, 'power:', power, 'power filter:', filters.motorPower);
      }
      return inRange;
    });
    console.log('🔍 After motor power filter:', filtered.length, 'motorbikes');
    ...
    filtered = filtered.filter((vehicle: unknown) => {
      ...
      if (!inRange) {
        console.log('🚫 Motorbike filtered out by top speed:', v.name, 'speed:', speed, 'speed filter:', filters.topSpeed);
      }
      return inRange;
    });
    console.log('🔍 After top speed filter:', filtered.length, 'motorbikes');
    ...
    filtered = filtered.filter((vehicle: unknown) => {
      ...
      if (!inRange) {
        console.log('🚫 Motorbike filtered out by acceleration:', v.name, 'acceleration:', acceleration, 'acceleration filter:', filters.acceleration);
      }
      return inRange;
    });
    console.log('🔍 After acceleration filter:', filtered.length, 'motorbikes');
    ...
    console.log('🔍 Final filtered result:', filtered.length, 'motorbikes');
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`pt-16 transition-all duration-150 ${isSidebarOpen ? 'ml-64' : 'ml-0'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <h1 className="text-4xl font-light text-gray-900 mb-8">Xe máy điện VinFast</h1>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg p-6 shadow-sm">
                ...
                {compareList.length > 0 && (
                  <div className="mt-6">
                    <h4 className="text-md font-medium text-gray-900 mb-3">So sánh xe máy ({compareList.length}/3)</h4>
                    <div className="space-y-2">
            ...
            <div className="lg:col-span-3">
              ...
              {loading && (
                <div className="flex justify-center items-center py-12">
                  ...
                  <span className="ml-3 text-gray-600">Đang tải danh sách xe máy...</span>
                </div>
              )}
              ...
              {!loading && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {(() => {
                    console.log('🎯 Rendering motorbikes:', filteredVehicles.length, 'motorbikes');
                    return null;
                  })()}
              ...
              {!loading && !error && filteredVehicles.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Không tìm thấy xe máy nào</h3>
                        ...
                        {compareList.map((vehicle) => {
                          ...
                          return (
                            <td key={v._id as string || v.id as string} className="p-6 text-center">
                              <img src={((v.images as string[]) || [])[0] || '/placeholder-motorbike.jpg'} alt={v.name as string} className="w-24 h-18 object-cover mx-auto rounded-lg shadow-sm" />
                            </td>
                          );
                        })}
                      </tr>
                      ...
                      return (
                        <div key={v._id as string || v.id as string} className="flex space-x-2">
                          ...
                          <button
                            onClick={() => {
                              setShowCompareModal(false);
                              handleDeposit(v._id as string || v.id as string);
                            }}
