...
export const OrderManagement: React.FC = () => {
  ...
  const [selectedOrderForDeposit, setSelectedOrderForDeposit] =
  ...
  const statusOptions = [
    {value: "pending", label: "Chờ (Cọc/Thanh toán)", color: "warning"},
  ...
  const getStatusChip = (status: string) => {
    ...
    const option = statusOptions.find(
      (opt) => opt.value === normalizedStatus || opt.value === status
    );
    if (option) {
      return (
        <Chip
        ...
        if (response && response.success) {
          ...
          if (response.data) {
            if (response.data.data && Array.isArray(response.data.data)) {
              ...
              if (response.data.pagination) {
                ...
                console.log(
                  "📊 Using direct pagination fields:",
                  paginationData
                );
          ...
          ordersData.forEach((order: any) => {
            if (
              ...
              typeof order.salesperson_id === "string"
            ) {
          ...
          if (salespersonIdsToFetch.size > 0) {
            console.log(
              "🔍 Fetching salesperson info for IDs:",
              Array.from(salespersonIdsToFetch)
            );
            try {
              const fetchPromises = Array.from(salespersonIdsToFetch).map(
                async (id) => {
                  try {
                    ...
                    if (userResponse.success && userResponse.data) {
                      ...
                      const fullName =
                        (user as any).full_name || user.name || "Unknown";
          ...
          const processedOrders = ordersData.map((order: any) => {
            ...
            if (
              ...
              order.salesperson_id &&
              typeof order.salesperson_id === "object" &&
              order.salesperson_id.full_name
            ) {
              ...
              const fetchedSalesperson = salespersonMap.get(
                order.salesperson_id
              );
          ...
          console.log(
            "📊 Setting orders state with:",
            processedOrders.length,
            "orders"
          );

          // Debug: Check for confirmed orders
          const confirmedOrders = processedOrders.filter(
            (order) => order.status === "confirmed"
          );
          console.log(
            "🔍 Confirmed orders found:",
          ...
          const deletedOrders = processedOrders.filter(
            (order) => (order as any).is_deleted
          );
          console.log(
            "🗑️ Deleted orders found:",
            deletedOrders.length,
            deletedOrders.map((o) => ({
              id: o._id,
              code: o.code,
              is_deleted: (o as any).is_deleted,
            }))
          );
          ...
          console.log(
            ...
            ordersWithCancelledStatus.length
          );
          if (ordersWithCancelledStatus.length > 0) {
            console.log("📦 Sample order:", {
              ...
              dealership_id_type:
                typeof ordersWithCancelledStatus[0].dealership_id,
              salesperson_id_type:
                typeof ordersWithCancelledStatus[0].salesperson_id,
            });
          }
          ...
          if (user?.role === "dealer_manager") {
            ...
            console.log(
              "🔍 Manager filtering by dealership_id:",
              userDealershipId
            );
            filteredOrders = ordersWithCancelledStatus.filter((order) => {
              ...
              const orderDealershipId =
                typeof order.dealership_id === "object" &&
                order.dealership_id !== null
                  ? (order.dealership_id as any)?._id ||
                    (order.dealership_id as any)?.id
                  : order.dealership_id;
              const belongsToUserDealership =
                orderDealershipId === userDealershipId;
              if (!belongsToUserDealership) {
                console.log(
                  "🚫 Manager filtering out order from different dealership:",
                  {
            ...
            console.log(
              `🔍 Manager filtering: ${ordersWithCancelledStatus.length} → ${filteredOrders.length} orders`
            );
          } else if (user?.role === "dealer_staff") {
            // Staff: API /api/orders/yourself đã filter rồi, không cần filter thêm
            // Chỉ log để debug
            console.log(
              "🔍 Staff: API đã filter, không filter thêm. Orders count:",
              ordersWithCancelledStatus.length
            );
          ...
          const totalFromAPI =
            ...
            (response.data as any)?.totalRecords ||
          ...
          const roleMessage =
            user?.role === "dealer_manager"
              ...
              : "User";
          setSnackbarMessage(
            `${roleMessage}: Đã tải ${filteredOrders.length} đơn hàng${filterMessage}`
          );
  ...
  const handleDepositPayment = (order: Order) => {
    console.log("💰 Deposit payment clicked for order:", {
      id: order._id,
      code: order.code,
      status: order.status,
    });
  ...
  const handleDeleteOrder = async (order: Order) => {
    ...
    const isFullyPaid =
      ...
      order.paid_amount >= (order.final_amount || 0);
    if (isFullyPaid) {
      await Swal.fire({
    ...
    const {value: formValues} = await Swal.fire({
            ...
            hasDeposit
              ? `<div style="background: #fff7e6; border: 1px solid #ffd591; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
                 <p style="margin: 0 0 12px 0; font-size: 15px; color: #262626;">
                   Đơn hàng <strong style="color: #1890ff;">${
                     order.code
                   }</strong> đã có tiền cọc.
                 </p>
      ...
      preConfirm: () => {
        const reason = (
          document.getElementById("cancellation_reason") as HTMLTextAreaElement
        )?.value?.trim();
        if (!reason) {
        ...
        if (reason.length < 10) {
          Swal.showValidationMessage(
            "Lý do hủy đơn hàng phải có ít nhất 10 ký tự"
          );
          return false;
        }
        return reason;
      ...
      await Swal.fire({
        ...
        html: hasDeposit
          ? `
            ...
            <p style="margin-top: 10px;">
              <strong>Số tiền đã hoàn lại:</strong> ${formatCurrency(
                order.paid_amount || 0
              )}
            </p>
      ...
      if (daysSinceDelivery < 1) {
        ...
        await Swal.fire({
          ...
          html: `
            ...
            <p style="margin-top: 10px;">
              <strong>Ngày giao xe:</strong> ${deliveryDateTime.format(
                "DD/MM/YYYY HH:mm"
              )}<br/>
          ...
          <TableContainer sx={{maxHeight: "calc(100vh - 350px)"}}>
            <Table stickyHeader aria-label="Bảng đơn hàng" size="small">
              <TableHead>
                <TableRow
                  sx={{
                    "& th": {fontWeight: "bold", backgroundColor: "#f4f6f8"},
                  }}
                >
                ...
                {loading ? (
                  ...
                  orders.map((order) => (
                    <TableRow hover key={order._id}>
                      ...
                      <TableCell align="center">
                        {getPaymentChip(order.payment_method)}
                      </TableCell>
                      <TableCell align="center">
                        ...
                        >
                          ...
                          {order.contract_signed && (
                            <Tooltip title="Xem hợp đồng">
                              <IconButton
                                onClick={() => {
                                  setSnackbarMessage(
                                    "Tính năng xem hợp đồng đang được phát triển"
                                  );
                          ...
                          {order.status === "confirmed" && (
                            <Tooltip title="Đặt cọc">
                              <IconButton
                                onClick={() => {
                                  console.log(
                                    "💰 Deposit button clicked for:",
                                    {code: order.code, status: order.status}
                                  );
                          ...
                          {(() => {
                            ...
                            if (order.status === "waiting_vehicle_request") {
                              ...
                              const orderRequestId = (order as any)
                                .order_request_id;
                              ...
                              if (orderRequest) {
                                ...
                                const requestVehicles =
                                  (orderRequest as any).request_vehicles || [];
                                const requestVehicleId = (orderRequest as any)
                                  .request_vehicle_id;

                                // Nếu có request_vehicles array, kiểm tra xem có request nào đã delivered không
                                if (
                                  Array.isArray(requestVehicles) &&
                                  requestVehicles.length > 0
                                ) {
                                  // Kiểm tra xem có request_vehicle nào đã delivered không
                                  return requestVehicles.some(
                                    (rv: any) => rv.status === "delivered"
                                  );
                          ...
                          {(() => {
                            ...
                            if (deliveryDate) {
                              const deliveryDateTime = dayjs(deliveryDate);
                              const now = dayjs();
                              const daysSinceDelivery = now.diff(
                                deliveryDateTime,
                                "day"
                              );
                              const hoursSinceDelivery = now.diff(
                                deliveryDateTime,
                                "hour"
                              );
