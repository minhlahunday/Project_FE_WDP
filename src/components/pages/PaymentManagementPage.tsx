...
> = () => {
  ...
  const [generatingContract, setGeneratingContract] = useState<string | null>(
    null
  );
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const errorMessage =
        (error as Error)?.message || "Lỗi khi tải danh sách đơn hàng";
  ...
  const getOrderStatusTag = (status: string) => {
    ...
    if (statusInfo) {
      return (
        <Tag style={statusInfo.style} className="status-tag">
          {statusInfo.text}
        </Tag>
      );
    }
  ...
  const columns: ColumnsType<Order> = [
    ...
    },
    {
      ...
      render: (_: unknown, record: Order) => {
        ...
        const name = customer?.full_name || "N/A";
    ...
    },
    {
      ...
      render: (_: unknown, record: Order) => {
        ...
        return <Tooltip title={phone}>{phone}</Tooltip>;
      },
    },
    {
      title: "Tổng tiền",
